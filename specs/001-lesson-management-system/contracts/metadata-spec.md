...
If new fields are needed in the future:
1. Add to optional fields first (with defaults)
2. Document rationale for new field
3. Update all existing lessons to include new field
4. Make required only after migration complete

### Removing Metadata Fields

Fields should be deprecated gradually:
1. Mark as "deprecated" in documentation
2. Continue accepting in parsing (backward compatible)
3. Stop including in new lessons
4. Remove entirely after 2-3 release cycles

### Default Values

If metadata field is missing:
- `status`: Defaults to `"published"`
- `author`: Defaults to repository maintainers
- `prerequisites`: Defaults to empty list
- `updated`: Not set if not provided (creation date used)
